...
That blue is safe, and that safety has a price. A blue palette will be accepted, and it will look like all the others. On this instrument's wheel, blue is one of the six anchors, facing red-orange. If you want to be universal, start there. If you want to be remembered, look the other way.`,refs:[{n:`YouGov. "Why is blue the world's favourite colour?", 2015.`,u:"https://yougov.co.uk/topics/lifestyle/articles-reports/2015/05/12/why-blue-worlds-favorite-colour"},{n:"Berlin, B.; Kay, P. *Basic Color Terms: Their Universality and Evolution*. Berkeley: University of California Press, 1969."},{n:"Pastoureau, M. *Blue: The History of a Color*. Princeton: Princeton University Press, 2001.",u:"https://press.princeton.edu/books/hardcover/9780691090504/blue"},{n:"Goethe, J. W. von. *Zur Farbenlehre*, § 778–785. Tübingen: Cotta, 1810.",u:"https://www.deutschestextarchiv.de/book/show/goethe_farbenlehre01_1810"},{n:"Kandinsky, W. *Über das Geistige in der Kunst*. Munich: Piper, 1911. English translation at Project Gutenberg.",u:"https://www.gutenberg.org/ebooks/5321"},{n:'Tate. "International Klein Blue".',u:"https://www.tate.org.uk/art/artists/yves-klein-1418"},{n:'Pantone. "Color of the Year 2020: Classic Blue".',u:"https://www.pantone.com/color-of-the-year/2020"}],seo:{kw:"world favourite colour blue",title:"One colour that unites the world: why blue is the favourite everywhere",desc:"Polls, languages, flags and Goethe: why blue is the most shared colour in the world and what that means for anyone designing palettes.",outline:["A colour without a name","What Goethe saw in blue","Why it unites","What this says to anyone who designs"],links:["Colour","Trends"]},music:{title:"Kind of Blue",artist:"Miles Davis",why:"The most listened-to record in jazz, and the bluest. Modal, calm, deep: everything the text says about the colour.",q:"Miles Davis Kind of Blue"}}}],W=[...D,...J,...F],m=a=>h()?a.en:a.pt,v=a=>new Date(a+"T12:00:00").toLocaleDateString(j(),{day:"numeric",month:"long",year:"numeric"}),S=a=>a.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,""),q=a=>a.replace(/^##\s+/gm,"").replace(/^>\s+/gm,"").replace(/\*\*?([^*]+)\*\*?/g,"$1").replace(/\s\[\d+\]/g,""),f=a=>location.origin+location.pathname+"#c/"+a.slug,i={topics:new Set,q:"",open:null,scale:1,dys:!1};try{i.scale=+(localStorage.getItem("fk-artscale")||1)||1,i.dys=localStorage.getItem("fk-artdys")==="1"}catch{}function B(a){const e=o=>r(o).replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/\*([^*]+)\*/g,"<em>$1</em>").replace(/\s\[(\d+)\]/g,' <sup class="ref"><a href="#ref-$1" aria-label="'+s("referência {n}",{n:"$1"})+'">$1</a></sup>');return a.split(/\n\s*\n/).map(o=>(o=o.trim(),o?o.startsWith("## ")?`<h2>${e(o.slice(3))}</h2>`:o.startsWith("> ")?`<blockquote class="pull">${e(o.slice(2))}</blockquote>`:`<p>${e(o)}</p>`:"")).join(`
`)}function U(a){if(i.topics.size&&!a.topics.some(t=>i.topics.has(t)))return!1;if(!i.q)return!0;const e=m(a),o=S([e.title,e.dek,e.kicker,q(e.body),e.refs.map(t=>t.n).join(" ")].join(" "));return i.q.split(/\s+/).filter(Boolean).every(t=>o.includes(S(t)))}const k=a=>{const e=G.find(o=>o.k===a);return h()?e.en:e.pt};function w(){const a=W.filter(U).sort((e,o)=>o.date.localeCompare(e.date));d("magTopics").innerHTML=G.map(e=>`<button class="pill" data-k="${e.k}" aria-pressed="${i.topics.has(e.k)}">${r(h()?e.en:e.pt)}</button>`).join(""),y(d("magTopics"),"button").forEach(e=>e.onclick=()=>{const o=e.dataset.k;i.topics.has(o)?i.topics.delete(o):i.topics.add(o),w()}),d("magCount").textContent=a.length===1?s("1 artigo"):s("{n} artigos",{n:a.length}),d("magList").innerHTML=a.length?a.map((e,o)=>{const t=m(e);return`<article class="magcard${o===0?" lead":""}" data-tone="${["accent","card","deep","ink","card"][o%5]}">
      <button class="magopen" data-s="${e.slug}">
        <span class="kicker">${r(t.kicker)}</span>
        <span class="magtitle">${r(t.title)}</span>
        <span class="magdek">${r(t.dek)}</span>
        <span class="magmeta"><time datetime="${e.date}">${r(v(e.date))}</time> · ${s("{n} min de leitura",{n:e.min})} · ${e.topics.map(k).join(", ")}</span>
      </button></article>`}).join(""):`<p class="sm">${s("Nada encontrado com esses filtros.")}</p>`,y(d("magList"),".magopen").forEach(e=>e.onclick=()=>L(e.dataset.s))}const _=document.title;function p(a,e,o){let t=document.head.querySelector(`meta[${e}="${a}"]`);if(o===null){t&&t.dataset.art&&t.remove();return}t||(t=document.createElement("meta"),t.setAttribute(e,a),t.dataset.art="1",document.head.appendChild(t)),t.content=o}function E(a){const e=document.getElementById("ldArticle");e&&e.remove();const o=document.head.querySelector('link[rel="canonical"]');if(!a){document.title=_,p("description","name",null),p("og:title","property",null),p("og:description","property",null),o&&o.dataset.art&&o.remove();return}const t=m(a);document.title=t.seo.title+" · Auge",p("description","name",t.seo.desc),p("og:title","property",t.seo.title),p("og:description","property",t.seo.desc),p("og:type","property","article");const l=o||Object.assign(document.createElement("link"),{rel:"canonical"});l.href=f(a),l.dataset.art="1",o||document.head.appendChild(l);const c=document.createElement("script");c.type="application/ld+json",c.id="ldArticle",c.textContent=JSON.stringify({"@context":"https://schema.org","@type":"Article",headline:t.title,description:t.seo.desc,datePublished:a.date,inLanguage:h()?"en-GB":"pt-BR",keywords:[t.seo.kw,...a.topics.map(k)].join(", "),author:{"@type":"Organization",name:"Auge"},publisher:{"@type":"Organization",name:"Auge"},mainEntityOfPage:f(a),wordCount:q(t.body).split(/\s+/).length}),document.head.appendChild(c)}let g=null;const T=()=>typeof speechSynthesis<"u"?speechSynthesis:null;function M(){const a=T();if(!a)return[];const e=h()?"en":"pt",o=a.getVoices(),t=o.filter(l=>l.lang.toLowerCase().startsWith(e));return t.length?t:o}function O(){const a=document.getElementById("artVoice");if(!a)return;const e=M();a.innerHTML=e.length?e.map((o,t)=>`<option value="${t}">${r(o.name)} (${r(o.lang)})</option>`).join(""):`<option value="">${s("Nenhuma voz instalada")}</option>`}function V(a){const e=T();if(!e)return $(s("Este navegador não oferece leitura em voz"));if(e.speaking&&e.paused){e.resume(),b("playing");return}if(e.speaking){e.pause(),b("paused");return}const o=m(a),t=[o.title,o.dek,q(o.body)].join(`. 
`);g=new SpeechSynthesisUtterance(t);const l=M(),c=d("artVoice"),n=l[+c.value];n&&(g.voice=n),g.lang=h()?"en-GB":"pt-BR",g.rate=+d("artRate").value,g.onend=()=>b("idle"),g.onerror=()=>b("idle"),e.cancel(),e.speak(g),b("playing")}function x(){const a=T();a&&a.cancel(),b("idle")}function b(a){const e=document.getElementById("artPlay");e&&(e.textContent=a==="playing"?s("Pausar"):a==="paused"?s("Continuar"):s("Ouvir"),e.setAttribute("aria-pressed",String(a==="playing")))}function N(a){const e=m(a);return`# ${e.title}

_${e.dek}_

${v(a.date)} · ${s("{n} min de leitura",{n:a.min})} · ${a.topics.map(k).join(", ")}
...
${f(a)}
`}function K(a){return q(N(a)).replace(/^#+\s+/gm,"").replace(/^_|_$/gm,"")}function Y(a){const e=m(a);return`<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word"><head><meta charset="utf-8"><title>${r(e.title)}</title>
...
<h1>${r(e.title)}</h1><p class="dek">${r(e.dek)}</p><p>${r(v(a.date))} · ${s("{n} min de leitura",{n:a.min})}</p>${B(e.body)}
<h2>${s("Referências")}</h2><ol>${e.refs.map(o=>`<li>${r(o.n)}${o.u?` <a href="${o.u}">${o.u}</a>`:""}</li>`).join("")}</ol>
<h2>${s("Música para ler")}</h2><p>${r(e.music.title)} — ${r(e.music.artist)}. ${r(e.music.why)}</p><hr><p><small>${r(s(C))}</small></p><p><small>${f(a)}</small></p></body></html>`}const C="Este texto foi cocriado entre uma pessoa e uma ferramenta de IA, como parte de um experimento de aprendizagem, criação e desenvolvimento. Não guardamos nenhum dado e nada daqui será usado como fonte de marketing.";function Z(a,e){const o=m(a),t=f(a),l=o.title+" — "+t,c=encodeURIComponent,n={facebook:`https://www.facebook.com/sharer/sharer.php?u=${c(t)}`,whatsapp:`https://wa.me/?text=${c(l)}`,linkedin:`https://www.linkedin.com/sharing/share-offsite/?url=${c(t)}`,instagram:"https://www.instagram.com/"};if(e==="instagram"){const u=navigator;if(u.share){u.share({title:o.title,text:o.dek,url:t}).catch(()=>{});return}I(t,s("Link copiado — o Instagram não recebe links de fora; cole na sua publicação"))}window.open(n[e],"_blank","noopener")}function L(a){const e=W.find(n=>n.slug===a);if(!e)return;const o=m(e);i.open=a,x(),location.hash!=="#c/"+a&&history.replaceState(null,"","#c/"+a);const t=d("magArt");t.hidden=!1,d("magHome").hidden=!0,t.style.setProperty("--artscale",String(i.scale)),t.classList.toggle("dys",i.dys),t.innerHTML=`<div class="arttools no-print">
    ...
    <article class="mag" lang="${h()?"en-GB":"pt-BR"}">
      <header class="maghead">
        <p class="kicker">${r(o.kicker)}</p>
        <h1>${r(o.title)}</h1>
        <p class="dek">${r(o.dek)}</p>
        <p class="magmeta"><time datetime="${e.date}">${s("Publicado em {d}",{d:r(v(e.date))})}</time> · ${s("{n} min de leitura",{n:e.min})} · ${e.topics.map(n=>`<span class="pill">${r(k(n))}</span>`).join(" ")}</p>
      </header>
      ...
      <div class="magbody">${B(o.body)}</div>
      ...
      <section class="magrefs"><h2>${s("Referências")}</h2>
        <ol>${o.refs.map((n,u)=>`<li id="ref-${u+1}">${r(n.n).replace(/\*([^*]+)\*/g,"<em>$1</em>")}${n.u?` <a href="${n.u}" target="_blank" rel="noopener">${r(n.u.replace(/^https?:\/\//,""))}</a>`:""}</li>`).join("")}</ol>
      ...
      <details class="magseo no-print"><summary>${s("Estratégia SEO deste artigo")}</summary>
        <dl>
          ...
          <dt>${s("Endereço")}</dt><dd><code>${r(f(e))}</code></dd>
      ...
      <p class="magdisc">${s(C)}</p>
    </article>`,d("artBack").onclick=()=>H(),d("artMinus").onclick=()=>c(i.scale-.1),d("artPlus").onclick=()=>c(i.scale+.1),d("artDys").onclick=()=>{i.dys=!i.dys,t.classList.toggle("dys",i.dys),d("artDys").setAttribute("aria-pressed",String(i.dys));try{localStorage.setItem("fk-artdys",i.dys?"1":"0")}catch{}},d("artPlay").onclick=()=>V(e),d("artStop").onclick=x,O();const l=T();l&&(l.onvoiceschanged=O),y(t,"[data-dl]").forEach(n=>n.onclick=()=>{const u=n.dataset.dl,A=e.slug+(h()?"-en":"-pt");if(u==="md")return z(A+".md",N(e),"text/markdown");if(u==="txt")return z(A+".txt",K(e),"text/plain");if(u==="doc")return z(A+".doc",Y(e),"application/msword");window.print()}),y(t,"[data-sh]").forEach(n=>n.onclick=()=>n.dataset.sh==="copy"?I(f(e),s("Link copiado")):Z(e,n.dataset.sh)),E(e);try{window.scrollTo({top:0,behavior:"instant"})}catch{}function c(n){i.scale=Math.min(1.6,Math.max(.8,Math.round(n*10)/10)),t.style.setProperty("--artscale",String(i.scale));try{localStorage.setItem("fk-artscale",String(i.scale))}catch{}}}function H(){i.open=null,x(),d("magArt").hidden=!0,d("magArt").innerHTML="",d("magHome").hidden=!1,E(null),location.hash.startsWith("#c/")&&history.replaceState(null,"","#c")}function P(){const a=location.hash.match(/^#c(?:\/([a-z0-9-]+))?$/);return a?(R("cont"),a[1]?L(a[1]):i.open&&H(),!0):!1}function X(){const a=d("magSearch");let e=0;a.oninput=()=>{clearTimeout(e),e=window.setTimeout(()=>{i.q=a.value.trim(),w()},120)},d("magClear").onclick=()=>{a.value="",i.q="",i.topics.clear(),w()},w(),window.addEventListener("hashchange",P),P()}export{X as initContents,B as mdHtml,P as openFromHash};
