...
That blue is safe, and that safety has a price. A blue palette will be accepted, and it will look like all the others. On this instrument's wheel, blue is one of the six anchors, facing red-orange. If you want to be universal, start there. If you want to be remembered, look the other way.`,refs:[{n:`YouGov. "Why is blue the world's favourite colour?", 2015.`,u:"https://yougov.co.uk/topics/lifestyle/articles-reports/2015/05/12/why-blue-worlds-favorite-colour"},{n:"Berlin, B.; Kay, P. *Basic Color Terms: Their Universality and Evolution*. Berkeley: University of California Press, 1969."},{n:"Pastoureau, M. *Blue: The History of a Color*. Princeton: Princeton University Press, 2001.",u:"https://press.princeton.edu/books/hardcover/9780691090504/blue"},{n:"Goethe, J. W. von. *Zur Farbenlehre*, § 778–785. Tübingen: Cotta, 1810.",u:"https://www.deutschestextarchiv.de/book/show/goethe_farbenlehre01_1810"},{n:"Kandinsky, W. *Über das Geistige in der Kunst*. Munich: Piper, 1911. English translation at Project Gutenberg.",u:"https://www.gutenberg.org/ebooks/5321"},{n:'Tate. "International Klein Blue".',u:"https://www.tate.org.uk/art/artists/yves-klein-1418"},{n:'Pantone. "Color of the Year 2020: Classic Blue".',u:"https://www.pantone.com/color-of-the-year/2020"}],seo:{kw:"world favourite colour blue",title:"One colour that unites the world: why blue is the favourite everywhere",desc:"Polls, languages, flags and Goethe: why blue is the most shared colour in the world and what that means for anyone designing palettes.",outline:["A colour without a name","What Goethe saw in blue","Why it unites","What this says to anyone who designs"],links:["Colour","Trends"]},music:{title:"Kind of Blue",artist:"Miles Davis",why:"The most listened-to record in jazz, and the bluest. Modal, calm, deep: everything the text says about the colour.",q:"Miles Davis Kind of Blue"}}}],L=[...re,...te,...se],b=a=>w()?a.en:a.pt,P=a=>new Date(a+"T12:00:00").toLocaleDateString(ae(),{day:"numeric",month:"long",year:"numeric"}),H=a=>a.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,""),I=a=>a.replace(/^##\s+/gm,"").replace(/^>\s+/gm,"").replace(/\*\*?([^*]+)\*\*?/g,"$1").replace(/\s\[\d+\]/g,""),T=a=>location.origin+location.pathname+"#c/"+a.slug,D=[{k:"essays",pt:"Ensaios"},{k:"lexicon",pt:"Léxico",caption:"Termos compartilhados entre Teoria e Tendências — em preparação"},{k:"archive",pt:"Arquivo",caption:"Edições encadernadas de Tendências — em preparação"},{k:"readings",pt:"Leituras",caption:"Indicações externas selecionadas — em preparação"}],r={topics:new Set,q:"",open:null,scale:1,dys:!1,module:"essays",view:"theme"};try{r.scale=+(localStorage.getItem("fk-artscale")||1)||1,r.dys=localStorage.getItem("fk-artdys")==="1"}catch{}function U(a){const e=t=>i(t).replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/\*([^*]+)\*/g,"<em>$1</em>").replace(/\s\[(\d+)\]/g,' <sup class="ref"><a href="#ref-$1" aria-label="'+s("referência {n}",{n:"$1"})+'">$1</a></sup>');let o=0;return a.split(/\n\s*\n/).map(t=>{if(t=t.trim(),!t)return"";if(t.startsWith("## ")){const d=t.slice(3);return`<h2 id="${"h-"+ ++o+"-"+_(d)}">${e(d)}</h2>`}return t.startsWith("> ")?`<blockquote class="pull">${e(t.slice(2))}</blockquote>`:`<p>${e(t)}</p>`}).join(`
`)}function _(a){return a.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")}function ie(a){let e=0;const o=[];for(const t of a.split(/\n\s*\n/)){const d=t.trim();if(d.startsWith("## ")){const u=d.slice(3);o.push({id:"h-"+ ++e+"-"+_(u),text:u})}}return o}const R=new Map;function ne(a){const e=b(a),o=a.slug+(w()?"en":"pt");let t=R.get(o);if(t===void 0){const d=e.body.match(/^>\s+(.+)$/m);t=d?d[1]:e.dek,R.set(o,t)}return t}function V(a,e=16){const o=(e-3)/2,t=2*Math.PI*o,d=Math.min(1,a/12),u=t*(1-d);return`<svg class="dial" width="${e}" height="${e}" viewBox="0 0 ${e} ${e}" aria-hidden="true">
    <circle cx="${e/2}" cy="${e/2}" r="${o}" fill="none" stroke="var(--rule2)" stroke-width="2"/>
    <circle cx="${e/2}" cy="${e/2}" r="${o}" fill="none" stroke="currentColor" stroke-width="2" stroke-dasharray="${t}" stroke-dashoffset="${u}" transform="rotate(-90 ${e/2} ${e/2})"/>
  </svg>`}function de(a){if(r.topics.size&&!a.topics.some(t=>r.topics.has(t)))return!1;if(!r.q)return!0;const e=b(a),o=H([e.title,e.dek,e.kicker,I(e.body),e.refs.map(t=>t.n).join(" ")].join(" "));return r.q.split(/\s+/).filter(Boolean).every(t=>o.includes(H(t)))}const O=a=>{const e=M.find(o=>o.k===a);return w()?e.en:e.pt},ce=[...L].sort((a,e)=>a.date.localeCompare(e.date)),le=a=>"E·"+String(ce.findIndex(e=>e.slug===a.slug)+1).padStart(2,"0");function W(a,e,o){const t=b(a),d=ne(a);return`<article class="magcard${o?" lead":""}" data-tone="${o?"accent":["card","deep","ink","card","accent"][e%5]}">
      <button class="magopen" data-s="${a.slug}">
        ${o?"":`<span class="plate" aria-hidden="true">${le(a)}</span>`}
        <span class="kicker">${i(t.kicker)}</span>
        <span class="magtitle">${i(t.title)}</span>
        <span class="magdek">${i(t.dek)}</span>
        <span class="magquote">“${i(d)}”</span>
        <span class="magmeta">${V(a.min)}<time datetime="${a.date}">${i(P(a.date))}</time> · ${s("{n} min de leitura",{n:a.min})} · ${a.topics.map(O).join(", ")}</span>
      </button></article>`}function ue(){const a=l("modTabs");a.setAttribute("aria-label",s("Módulos")),a.innerHTML=D.map(e=>{const o=e.k!=="essays";return`<button class="modtab" role="tab" data-m="${e.k}" aria-selected="${r.module===e.k}" ${o?'aria-disabled="true"':""}>
      <span>${i(s(e.pt))}</span>${o?`<span class="cap">${i(s(e.caption))}</span>`:""}</button>`}).join(""),v(a,".modtab").forEach(e=>e.onclick=()=>{const o=e.dataset.m;o===r.module||e.getAttribute("aria-disabled")==="true"||(r.module=o,x())})}function he(){const a=l("magViewToggle");a.setAttribute("aria-label",s("Ordenar por")),a.innerHTML=`<button role="radio" data-v="theme" aria-checked="${r.view==="theme"}">${s("Por tema")}</button>
    <button role="radio" data-v="date" aria-checked="${r.view==="date"}">${s("Por data")}</button>`,v(a,"button").forEach(e=>e.onclick=()=>{const o=e.dataset.v;o!==r.view&&(r.view=o,x())})}function me(a){const e=document.getElementById("topic-"+a);e&&(e.scrollIntoView({behavior:"smooth",block:"start"}),e.classList.add("hl"),e.classList.remove("off"),setTimeout(()=>e.classList.add("off"),600))}function x(){ue(),he();const a=l("magList");a.classList.toggle("dysoff",r.dys);const e=document.querySelector(".magfilters");if(r.module!=="essays"){e&&(e.hidden=!0);const n=D.find(h=>h.k===r.module);a.innerHTML=`<div class="modsoon"><b>${i(s(n.pt))}</b>${i(s(n.caption))}</div>`;return}e&&(e.hidden=!1),l("magTopics").innerHTML=M.map(n=>`<button class="pill" data-k="${n.k}" aria-pressed="${r.topics.has(n.k)}">${i(w()?n.en:n.pt)}</button>`).join(""),v(l("magTopics"),"button").forEach(n=>n.onclick=()=>{const h=n.dataset.k;if(r.view==="theme"){me(h);return}r.topics.has(h)?r.topics.delete(h):r.topics.add(h),x()});const o=L.filter(de);if(l("magCount").textContent=o.length===1?s("1 artigo"):s("{n} artigos",{n:o.length}),!o.length){a.innerHTML=`<p class="sm">${s("Nada encontrado com esses filtros.")}</p>`;return}const t=!r.q&&!r.topics.size,d=[...o].sort((n,h)=>h.date.localeCompare(n.date)),u=t?d[0]:null,m=u?d.filter(n=>n.slug!==u.slug):d,p=u?`<div class="maglist">${W(u,0,!0)}</div>`:"";let g;r.view==="date"?g=`<div class="maglist">${m.map((n,h)=>W(n,h,!1)).join("")}</div>`:g=M.map(n=>{const h=m.filter(f=>f.topics[0]===n.k).sort((f,c)=>c.date.localeCompare(f.date));return h.length?`<div class="topicgroup" id="topic-${n.k}"><h2>${i(O(n.k))}</h2><hr class="rule">
        <div class="maglist">${h.map((f,c)=>W(f,c,!1)).join("")}</div></div>`:""}).join(""),a.innerHTML=p+g,v(a,".magopen").forEach(n=>n.onclick=()=>Q(n.dataset.s))}const pe=document.title;function q(a,e,o){let t=document.head.querySelector(`meta[${e}="${a}"]`);if(o===null){t&&t.dataset.art&&t.remove();return}t||(t=document.createElement("meta"),t.setAttribute(e,a),t.dataset.art="1",document.head.appendChild(t)),t.content=o}function K(a){const e=document.getElementById("ldArticle");e&&e.remove();const o=document.head.querySelector('link[rel="canonical"]');if(!a){document.title=pe,q("description","name",null),q("og:title","property",null),q("og:description","property",null),o&&o.dataset.art&&o.remove();return}const t=b(a);document.title=t.seo.title+" · Auge",q("description","name",t.seo.desc),q("og:title","property",t.seo.title),q("og:description","property",t.seo.desc),q("og:type","property","article");const d=o||Object.assign(document.createElement("link"),{rel:"canonical"});d.href=T(a),d.dataset.art="1",o||document.head.appendChild(d);const u=document.createElement("script");u.type="application/ld+json",u.id="ldArticle",u.textContent=JSON.stringify({"@context":"https://schema.org","@type":"Article",headline:t.title,description:t.seo.desc,datePublished:a.date,inLanguage:w()?"en-GB":"pt-BR",keywords:[t.seo.kw,...a.topics.map(O)].join(", "),author:{"@type":"Organization",name:"Auge"},publisher:{"@type":"Organization",name:"Auge"},mainEntityOfPage:T(a),wordCount:I(t.body).split(/\s+/).length}),document.head.appendChild(u)}let k=null;const G=()=>typeof speechSynthesis<"u"?speechSynthesis:null;function Y(){const a=G();if(!a)return[];const e=w()?"en":"pt",o=a.getVoices(),t=o.filter(d=>d.lang.toLowerCase().startsWith(e));return t.length?t:o}function j(){const a=document.getElementById("artVoice");if(!a)return;const e=Y();a.innerHTML=e.length?e.map((o,t)=>`<option value="${t}">${i(o.name)} (${i(o.lang)})</option>`).join(""):`<option value="">${s("Nenhuma voz instalada")}</option>`}function ge(a){const e=G();if(!e)return oe(s("Este navegador não oferece leitura em voz"));if(e.speaking&&e.paused){e.resume(),A("playing");return}if(e.speaking){e.pause(),A("paused");return}const o=b(a),t=[o.title,o.dek,I(o.body)].join(`. 
`);k=new SpeechSynthesisUtterance(t);const d=Y(),u=l("artVoice"),m=d[+u.value];m&&(k.voice=m),k.lang=w()?"en-GB":"pt-BR",k.rate=+l("artRate").value,k.onend=()=>A("idle"),k.onerror=()=>A("idle"),e.cancel(),e.speak(k),A("playing")}function $(){const a=G();a&&a.cancel(),A("idle")}function A(a){const e=document.getElementById("artPlay");e&&(e.textContent=a==="playing"?s("Pausar"):a==="paused"?s("Continuar"):s("Ouvir"),e.setAttribute("aria-pressed",String(a==="playing")))}function Z(a){const e=b(a);return`# ${e.title}
...
${T(a)}
`}function fe(a){return I(Z(a)).replace(/^#+\s+/gm,"").replace(/^_|_$/gm,"")}function we(a){const e=b(a);return`<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word"><head><meta charset="utf-8"><title>${i(e.title)}</title>
...
<h1>${i(e.title)}</h1><p class="dek">${i(e.dek)}</p><p>${i(P(a.date))} · ${s("{n} min de leitura",{n:a.min})}</p>${U(e.body)}
<h2>${s("Referências")}</h2><ol>${e.refs.map(o=>`<li>${i(o.n)}${o.u?` <a href="${o.u}">${o.u}</a>`:""}</li>`).join("")}</ol>
<h2>${s("Música para ler")}</h2><p>${i(e.music.title)} — ${i(e.music.artist)}. ${i(e.music.why)}</p><hr><p><small>${i(s(N))}</small></p><p><small>${T(a)}</small></p></body></html>`}const N="Este texto foi cocriado entre uma pessoa e uma ferramenta de IA, como parte de um experimento de aprendizagem, criação e desenvolvimento. Não guardamos nenhum dado e nada daqui será usado como fonte de marketing.";function be(a,e){const o=b(a),t=T(a),d=o.title+" — "+t,u=encodeURIComponent,m={facebook:`https://www.facebook.com/sharer/sharer.php?u=${u(t)}`,whatsapp:`https://wa.me/?text=${u(d)}`,linkedin:`https://www.linkedin.com/sharing/share-offsite/?url=${u(t)}`,instagram:"https://www.instagram.com/"};if(e==="instagram"){const p=navigator;if(p.share){p.share({title:o.title,text:o.dek,url:t}).catch(()=>{});return}F(t,s("Link copiado — o Instagram não recebe links de fora; cole na sua publicação"))}window.open(m[e],"_blank","noopener")}let C=null,z=null,S=null;function Q(a){const e=L.find(n=>n.slug===a);if(!e)return;const o=b(e);r.open=a,$(),location.hash!=="#c/"+a&&history.replaceState(null,"","#c/"+a);const t=l("magHome"),d=l("magArt"),u=matchMedia("(prefers-reduced-motion: reduce)").matches,m=ie(o.body),p=m.map(n=>`<li><a href="#${n.id}" data-h="${n.id}"><span class="tick" aria-hidden="true"></span>${i(n.text)}</a></li>`).join(""),g=()=>{d.hidden=!1,t.hidden=!0,d.style.setProperty("--artscale",String(r.scale)),d.classList.toggle("dys",r.dys),d.innerHTML=`<div class="artprogress" id="artProgress" aria-hidden="true"></div>
    <div class="arttools no-print">
      ...
      <div class="artread" role="group" aria-label="${s("Ajustes de leitura")}">
        ...
        <button class="mini" id="artDys" aria-pressed="${r.dys}">${s("Leitura facilitada")}</button>
      </div></div>
    <div class="artgrid">
      ...
      <div>
    <article class="mag" lang="${w()?"en-GB":"pt-BR"}">
      <header class="maghead">
        <p class="kicker">${i(o.kicker)}</p>
        <h1>${i(o.title)}</h1>
        <p class="dek">${i(o.dek)}</p>
        <p class="magmeta"><time datetime="${e.date}">${s("Publicado em {d}",{d:i(P(e.date))})}</time> · <span id="artMinLeft">${s("{n} min de leitura",{n:e.min})}</span> · ${e.topics.map(c=>`<span class="pill">${i(O(c))}</span>`).join(" ")}</p>
        ${m.length?`<details class="artpage"><summary>${s("Nesta página")}</summary><ul>${m.map(c=>`<li><a href="#${c.id}">${i(c.text)}</a></li>`).join("")}</ul></details>`:""}
      </header>
      ...
      <aside class="magmusic">
        ...
        <p><b>${i(o.music.title)}</b> — ${i(o.music.artist)}</p>
        <p class="sm">${i(o.music.why)}</p>
      ...
      <section class="magrefs" id="magRefs"><h2>${s("Referências")}</h2>
        <ol>${o.refs.map((c,y)=>`<li id="ref-${y+1}">${i(c.n).replace(/\*([^*]+)\*/g,"<em>$1</em>")}${c.u?` <a href="${c.u}" target="_blank" rel="noopener">${i(c.u.replace(/^https?:\/\//,""))}</a>`:""}</li>`).join("")}</ol>
      ...
      <details class="magseo no-print"><summary>${s("Estratégia SEO deste artigo")}</summary>
        <dl>
          <dt>${s("Palavra-chave foco")}</dt><dd>${i(o.seo.kw)}</dd>
          <dt>${s("Título da página")}</dt><dd>${i(o.seo.title)} <span class="sm">(${o.seo.title.length}/60)</span></dd>
          <dt>${s("Descrição")}</dt><dd>${i(o.seo.desc)} <span class="sm">(${o.seo.desc.length}/160)</span></dd>
          <dt>${s("Endereço")}</dt><dd><code>${i(T(e))}</code></dd>
          <dt>${s("Estrutura de seções (H2)")}</dt><dd>${o.seo.outline.map(i).join(" · ")}</dd>
          <dt>${s("Ligações internas")}</dt><dd>${o.seo.links.map(i).join(", ")}</dd>
      ...
      <aside class="artmargin">${V(e.min,22)}<span class="sm">${s("{n} min de leitura",{n:e.min})}</span>
        <p class="sm"><b>${i(o.music.title)}</b> — ${i(o.music.artist)}</p>
        <a class="mini" href="#magRefs">${s("Ir às referências")}</a></aside>
    </div>`,l("artBack").onclick=()=>X(),f(),l("artMinus").onclick=()=>h(r.scale-.1),l("artPlus").onclick=()=>h(r.scale+.1),l("artDys").onclick=()=>{r.dys=!r.dys,d.classList.toggle("dys",r.dys),l("artDys").setAttribute("aria-pressed",String(r.dys));try{localStorage.setItem("fk-artdys",r.dys?"1":"0")}catch{}},l("artPlay").onclick=()=>ge(e),l("artStop").onclick=$,j();const n=G();n&&(n.onvoiceschanged=j),v(d,"[data-dl]").forEach(c=>c.onclick=()=>{const y=c.dataset.dl,E=e.slug+(w()?"-en":"-pt");if(y==="md")return B(E+".md",Z(e),"text/markdown");if(y==="txt")return B(E+".txt",fe(e),"text/plain");if(y==="doc")return B(E+".doc",we(e),"application/msword");window.print()}),v(d,"[data-sh]").forEach(c=>c.onclick=()=>c.dataset.sh==="copy"?F(T(e),s("Link copiado")):be(e,c.dataset.sh)),K(e);try{window.scrollTo({top:0,behavior:"instant"})}catch{}ye(e);function h(c){r.scale=Math.min(1.6,Math.max(.8,Math.round(c*10)/10)),d.style.setProperty("--artscale",String(r.scale));try{localStorage.setItem("fk-artscale",String(r.scale))}catch{}f()}function f(){const c=l("artMinus"),y=l("artPlus");c.disabled=r.scale<=.8,y.disabled=r.scale>=1.6}S=c=>{(c.ctrlKey||c.metaKey)&&(c.key==="+"||c.key==="="?(c.preventDefault(),l("artPlus").click()):c.key==="-"&&(c.preventDefault(),l("artMinus").click()))},document.addEventListener("keydown",S)};if(u){g();return}d.classList.add("artgone"),t.classList.add("artgone"),setTimeout(()=>{g(),requestAnimationFrame(()=>d.classList.remove("artgone"))},150)}function ye(a){const e=document.getElementById("artProgress"),o=document.querySelector(".maghead"),t=document.getElementById("magRefs"),d=v(document,".artrail a");let u=0;C=()=>{const m=performance.now();if(!(m-u<60)&&(u=m,e&&o&&t)){const p=o.getBoundingClientRect().top+window.scrollY,g=t.getBoundingClientRect().bottom+window.scrollY,n=Math.max(0,Math.min(1,(window.scrollY-p)/Math.max(1,g-p-window.innerHeight)));e.style.width=n*100+"%";const h=document.getElementById("railTime");if(h){const f=Math.max(0,Math.round(a.min*(1-n)));h.textContent=s("{n} min restantes",{n:f})}}},window.addEventListener("scroll",C,{passive:!0}),C(),d.length&&"IntersectionObserver"in window&&(z=new IntersectionObserver(m=>{for(const p of m)if(p.isIntersecting){const g=p.target.id;d.forEach(n=>n.toggleAttribute("aria-current",n.dataset.h===g)),d.forEach(n=>{n.dataset.h===g?n.setAttribute("aria-current","location"):n.removeAttribute("aria-current")})}},{rootMargin:"0px 0px -70% 0px"}),v(document,".magbody h2").forEach(m=>z.observe(m)))}function X(){r.open=null,$(),C&&(window.removeEventListener("scroll",C),C=null),z&&(z.disconnect(),z=null),S&&(document.removeEventListener("keydown",S),S=null),l("magArt").hidden=!0,l("magArt").innerHTML="",l("magHome").hidden=!1,l("magHome").classList.remove("artgone"),K(null),location.hash.startsWith("#c/")&&history.replaceState(null,"","#c")}function J(){const a=location.hash.match(/^#c(?:\/([a-z0-9-]+))?$/);return a?(ee("cont"),a[1]?Q(a[1]):r.open&&X(),!0):!1}function qe(){const a=l("magSearch");let e=0;a.oninput=()=>{clearTimeout(e),e=window.setTimeout(()=>{r.q=a.value.trim(),x()},120)},l("magClear").onclick=()=>{a.value="",r.q="",r.topics.clear(),x()},x(),window.addEventListener("hashchange",J),J()}export{qe as initContents,U as mdHtml,J as openFromHash};
